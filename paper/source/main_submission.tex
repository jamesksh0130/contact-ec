...
close the temporal gap. A Foldseek/TM-score-disjoint split further shows that
...
Our analysis makes six contributions. We provide a temporal label-shift audit for
...
sequence-disjoint evaluation. We decompose data recency and fine-tuning, finding that newer Swiss-Prot data helps
...
values are mean $\pm$ sample standard deviation across three seeds; other rows
are single-run or externally reported. Bold denotes the best value among our
SP-2018-trained models.}
\label{tab:main}
\scriptsize
...
families, but explicit structure does not by itself close the gap to strong
sequence-only systems trained on newer data. The strongest temporal
...
comparison and the Foldseek audit now include three-seed repeats, 3B, recent-data,
and end-to-end variants are still single-run analyses due to compute cost. Bootstrap and
...
generalisation from fold shift, label shift, data recency, backbone scale, and structure
availability.
