...
failure rather than a simple missing-input problem. A supplementary contact-pair
nearest-neighbor audit further shows that all evaluable temporal proteins have
high nearest training-set similarity in the contact-pair representation, so the
temporal results should not be interpreted as fold-disjoint structural
generalization. Macro F1 remains low due to the long-tailed Level-4 label space.
The temporal test is small and results are from single trained checkpoints, so
bootstrap and paired tests quantify sample-level uncertainty rather than
definitive rankings. Finally, closed-set models cannot solve novel EC discovery
without retrieval, open-set, or label-expansion mechanisms.
