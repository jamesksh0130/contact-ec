...
\keywords[Keywords]{enzyme commission number; contact map; protein language model;
...
HIT-EC predictions were obtained from the publicly released pretrained
checkpoint (\texttt{model.ckpt}) in the official HIT-EC repository, using the
authors' released inference configuration (vocabulary size 23, two attention
heads, output dimensions [7,\,72,\,268,\,4255], sigmoid threshold 0.4),
without any retraining or fine-tuning on our data.
...
than the +0.8\,pp gain from replacing ESM-2 650M with the 3B backbone,
...
Contact-EC is a decomposition probe rather than a homology replacement. We also
audited whether CLEAN-Contact~\citep{ayres2024}, a retrieval-based external
baseline, could be run directly on our temporal splits, but found that its
public reference database has near-full-length $\geq99\%$-identity matches
for 96.0\% of SP-2023-01 and 68.1\% of SP-2024 proteins (Supplementary
Section~S5), so a head-to-head comparison would largely measure self-retrieval
rather than generalization; we report the self-reported New-392/Price-149
numbers instead. Macro F1 remains low due to the long-tailed Level-4 label
...
CRediT author statement: SK: Conceptualization, Methodology,
Software, Validation, Formal Analysis, Investigation, Data Curation,
Visualization, Writing -- Original Draft, Writing -- Review \& Editing.
The author has read and approved the final manuscript.

\subsection*{Acknowledgements}
The author acknowledges Handong Global University for providing the GPU
compute resources used in this study, and the developers and curators of
UniProt, AlphaFold DB, ESM, EC-Bench, CLEAN, CLEAN-Contact, and HIT-EC for
making their resources available to the research community.
