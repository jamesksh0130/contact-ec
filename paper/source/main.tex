%% BMC Bioinformatics submission manuscript
...
yielding a $+$22.4\,pp advantage; a vocabulary-stratified analysis indicates that most
of HIT-EC's decrease is not explained by
...
Supplementary information is included as a separate file.%
}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.58\textheight,keepaspectratio]{fig1_overview_print.pdf}
...
hyper-parameter table). Level-4
micro F1 is the primary metric, with macro F1, weighted F1, rare-EC F1,
precision, and recall reported for interpretation. Hyper-parameters were fixed
before temporal testing.
...
$-$29.7\,pp (75\,\% of the total drop) remains after restricting to
vocabulary-covered proteins. Contact-EC is stable across all strata: 0.6965 on
...
while Contact-EC improves under this mapped evaluation, reversing the ranking seen on SP-2023-01.}
\label{tab:sp2024}
...
level (0.8471), indicating that vocabulary mismatch alone does not explain
the observed SP-2024 decrease. Stratified Contact-EC values use the canonical
...
(Supplementary seen-label aggregate table), showing that the fusion advantage is not solely a
consequence of unseen-label composition.
...
shuffling ($-$54.1\,pp) and random density matching ($-$50.5\,pp), indicating that
its performance depends on protein-specific contact topology rather than
contact-map density or protein-length proxies alone. The partial retention under
...
remaining $\sim$30\,pp persists in
...
topology providing a signal that may be complementary to sequence-derived features
...
\url{https://github.com/jamesksh0130/contact-ec}. Operating system:
