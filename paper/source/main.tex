...
\textbf{(C)} Contact-EC pipeline: ESM-2 (1280-d) and ResNet-50 contact-map
encoder (512-d) are fused via gated cross-attention before a flat FC
sigmoid head predicts all 1,938 Level-4 EC classes simultaneously.}
\label{fig:pipeline}
\end{figure}
...
\textbf{(A)} B3 (contact-map only) collapses to zero under all three
perturbations, confirming reliance on genuine fold topology.
...
revealing that its gated cross-attention routes to the ESM-2 branch when
